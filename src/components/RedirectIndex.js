...
class RedirectIndex extends React.Component {
	componentWillMount() {
		if (typeof window !== 'undefined') {
			...
			const urlContainsLocaleSlug = localesSlugs.filter(a => currentPathName.includes(a)).length > 0

			if (!urlContainsLocaleSlug) {
				if (currentPathName === '/') {
					const browserLangInTwoChars = getBrowserLanguage().substr(0, 2)
					const matchingLang = availableLocales.find(l => l === browserLangInTwoChars)
					const browserLangLocalizationExist = matchingLang && matchingLang.length > 0
					const preferredLang = browserLangLocalizationExist ? matchingLang : this.props.defaultLocale
